...
impl GenBot3 {
  ...
  fn create(&mut self, game_info: &GameInfo) {
    ...
    for n in 0..num_nodes {
      ...
      instance.input_indexes = indexes
        .choose_multiple(&mut rand::thread_rng(), instance.num_inputs)
        .cloned()
        .collect();
      self.nodes.push(instance);
      next_index += 1;
...
impl GameObject for GenBot3 {
  fn to_json(&self) -> serde_json::Value {
    // println!("RECIPE OUT: {}", self.get_recipe());
    serde_json::json!({
...
impl GamePlayer for GenBot3 {
  ...
  fn mutate(&mut self) {
    ...
    self.nodes[node_index].input_indexes = indexes
      .choose_multiple(&mut rand::thread_rng(), num_inputs)
      .cloned()
      .collect();
  }
}